...
const iconSrc = computed(() => {
    return `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${props.pokemon.pid}.png`
})
...
<template>
    <div class="pokemon-card flex justify-between border-2 rounded-xl p-4 text-left w-2/3 mx-auto mb-6 min-w-max">
        <div class="flex justify-start m-2 w-full">
            <div class="w-1/4">
                <img :src="iconSrc" :alt="props.pokemon.name" class="w-40">
            </div>
            <div class="w-full">
        ...
        <div class="w-64">
            <svg width="250" height="270" class="mt-2">
                <PolyGraph :values="pokemon.rawStats" :className="'rv'" :nature="props.pokemon.nature" />
                <PolyGraph :values="props.pokemon.evs" :className="'ev'" :nature="props.pokemon.nature" />
            </svg>
        </div>
