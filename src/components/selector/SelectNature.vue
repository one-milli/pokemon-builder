...
const handleChangeNature = () => {
    emit('changeNature', selectedNature.value)
}
</script>

<template>
    <select v-model="selectedNature" @change="handleChangeNature">
        <option v-for="nature in natures" :key="nature.label" :value="nature">{{ nature.label }}</option>
    </select>
</template>
